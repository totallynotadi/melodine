...
class Video(URIBase):
    ...
    def from_raw(cls, data: Dict) -> "Video":
        data["artists"] = [{"name": data.pop("author"), "id": data.pop("channelId")}]
    ...
    def artists(self) -> Union[Artist, "ytmusic.User"]:
        ...
        if len(self._artists) == 0:
            if self._data is None:
                self._get_data()
            self._artists = [
                {"name": self._data.pop("author"), "id": self._data.pop("channelId")}
            ]
    ...
    def duration(self) -> int:
        ...
        if isinstance(self._duration, str):
            if ":" in self._duration:
                ...
                self._duration = int(self._duration[0]) * 60 + int(self._duration[1])
            else:
    ...
    def get_recommendations(self, limit: int = 10) -> List["Track"]:
        ...
        earlier fetched recommended tracks can be accessed whith the `.recommendations` property
        """
