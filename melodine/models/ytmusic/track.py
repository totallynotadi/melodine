...
class Track(URIBase):
    ...
    def __init__(self, data: Dict, **kwargs) -> None:
        ...
        self.duration: int = data.get("duration_seconds", data.get("lengthSeconds", 0))
        ...
        self.images: List[Image] = [Image(**image) for image in (images or [])]

        artists: Any
        if "artists" in data:
            artists = data.get("artists")
        else:
            artists = [{"name": data.get("author"), "id": data.get("channelId")}]
    ...
    def from_id(cls, id: str) -> "Track":
        ...
        return cls(data=service.ytmusic.get_song(id)["videoDetails"])
    ...
    def get_recommendations(self, limit: int = 10) -> List["Track"]:
        ...
        earlier fetched recommended tracks can be accessed from the `.recommendations` property.
        """

        data = service.ytmusic.get_watch_playlist(
            self.id, f"RDAMVM{self.id}", limit=self._recs_offset + limit
        )
