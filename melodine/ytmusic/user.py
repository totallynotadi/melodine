...
class User(URIBase):
    ...
    def __init__(self, data: Dict, **kwargs) -> None:
        ...
        self.href: str = "https://music.youtube.com/channel/" + str(self.id or "")
    ...
    def total_uploads(self) -> int:
        """total number of tracks/videos uploaded by the user"""
        if self._total_uploads:
            return self._total_uploads

        self._total_uploads = service.yt.get_playlist_items(
            playlist_id=self.uploads_id, limit=1, return_json=True
    ...
    def total_playlists(self) -> int:
        ...
        self._total_playlists = service.yt.get_playlists(
            channel_id=self.id, limit=1, return_json=True
        )["pageInfo"]["totalResults"]
        return self._total_playlists
    ...
    def get_videos(self, limit: int = 10, reset=False) -> List[Video]:
        """Get a list of Videos from a User"""
        results = []
        data = {
            "nextPageToken": (
                "<placeholder>"
                if not self.__prev_page_token
                else self.__next_page_token
            )
        }

        if data.get("nextPageToken"):
            if data["nextPageToken"] == "<placeholder>" or reset:
                data = service.yt.get_playlist_items(
    ...
    def get_all_videos(self) -> List[Video]:
        ...
        while count < self.total_uploads:
            ...
            self._videos += [Video(video["snippet"]) for video in data["items"]]
    ...
    #         self._playlists += [Playlist(playlist["id"]) for playlist in data["items"]]
    #     return self._playlists
