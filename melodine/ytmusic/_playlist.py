...
so a simple try catch for when a artist owns a playlist should suffice.
"""
...
class Playlist(URIBase):
    ...
    def _get_data(self) -> None:
        self._data = service.ytmusic.get_playlist(self.id)
    ...
    def tracks(self) -> List[Union[Video, Track]]:
        ...
        self._tracks = list(filter(lambda track: track is not None, self._tracks))
        return self._tracks
    ...
    def owner(self) -> Union["ytmusic.User", YTMusicArtist]:
        ...
        if not isinstance(self._owner, (YTMusicArtist, User)):
            try:
                ...
                self._owner = (
                    User.from_id(self._owner)
                    if isinstance(self._owner, str)
                    else User(user, id=self._owner["id"])
                )
