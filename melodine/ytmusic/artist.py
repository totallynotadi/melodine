...
class YTMusicArtist(URIBase):
    ...
    def __init__(self, data: dict) -> None:
        ...
        self._images: List[Image] = [Image(**image) for image in data.get("images", [])]

        self._related_artists: List[Dict] = data.get("related", {}).get("results", [])
    ...
    def views(self) -> int:
        if self._views is None:
            if self._data is None:
                self._get_data()
            self._views = int(self._data["views"].split(" ")[0].replace(",", "_"))
        return self._views
